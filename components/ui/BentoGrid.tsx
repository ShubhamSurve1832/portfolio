...
export const BentoGridItem = ({
  ...
  const handleCopy = () =>{
    navigator.clipboard.writeText("shubhamsurve1832@gmail.com");
    setCopied(true)
  }
  return (
    <div
    ...
    >
      ...
      <div className={`${id === 6}&& flex justify-center h-full`}>
        <div className="w-full h-full absolute">
          {img && <Image width={10} height={10} src={img} alt={img} className={cn(imgClassName, "object-cover object-center")} />}
        </div>
        <div className={`absolute right-0 -bottom-5 ${id === 5 && "w-full opacity-80"}`}>
          {spareImg && <Image width={10} height={10} src={spareImg} alt={spareImg} className={"object-cover object-center w-full h-full"} />}
        </div>
        {id === 6 && 
        ...
        <div className={cn(titleClassName, 'group-hover/bento:translate-x-2 transition duration-200 relative md:h-full min-h-40 flex flex-col p-5 lg:p-10')}>
          ...
          {id === 3 && (
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
              ...
              <div className="flex flex-col-reverse gap-3 lg:gap-8">
                <span className="py-4 px-3 rounded-lg text-center bg-[#10132E]" />
                {['React.js', 'Next.js', 'Tailwind CSS'].map((item, index) => {
                  return (
                    <>
